/*
 * Copyright 2017 Lars Sønderby Jessen Partly based on code copied from Vaadin Framework: Copyright 2000-2016 Vaadin
 * Ltd. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance
...
public class AutoBinder<BEAN>
        extends ReflectionBinder<BEAN> {
     ...
     * using {@link ReflectionBinder#bind(HasValue, String)}.
     * <p>
     * For example:
     *
     * <pre>
    ...
    public void bindInstanceFields(Object objectWithMemberFields) {
        ...
        Integer numberOfBoundFields = getFieldsInDeclareOrder(objectClass).stream()
                ...
                .filter(memberField -> hasAnnotation(memberField, FormField.class))
    ...
    private boolean hasAnnotation(Field memberField, Class<? extends Annotation> annotationClass) {
        return memberField.isAnnotationPresent(annotationClass);
    }
    ...
    protected <T> void buildAndBind(Class<?> currentClazz, String path, List<Component> components,
        ...
        for (Field field : fields) {
            if ((field.getModifiers() & Modifier.STATIC) != 0)
                continue;
            else if (!hasAnnotation(field, FormField.class))
                continue;
    ...
    /**
     * Constructs UI form components based on properties in a POJO class and binds the components to the properties.
     * <p>
     * This method processes all non-static (Java) member fields in a class and tries to construct an UI component by
     * ... TODO This method processes all (Java) member fields whose type extends {@link HasValue} and that can be
     * mapped to a property id. Property name mapping is done based on the field name or on a @{@link PropertyId}
     * annotation on the field. All non-null unbound fields for which a property name can be determined are bound to the
     ...
     * </pre>
     *
     * This iterates over all properties in the class and tries to construct UI components based on the property. The
     * "car" and "car.engine" will be handled as nested properties.
     *
